...
class ConnectFourBoard(object):
    ...
    def __init__(self, board_array = None, connect_k = 4, longest_streak = False,board_already_won = None, modified_column = None, current_player = 1, previous_move = -1):
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(new_board, self.connect_k,self.longest_streak,board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id())

    def _is_win_from_cell(self, row, col):
        """ Determines if there is a winning set of k connected nodes containing the specified cell """
        return ( self._max_length_from_cell(row, col) >= self.connect_k )
    ...
    def _contig_vector_cells(self, row, col, direction, potential=False):
        """
        Starting in the specified cell and going a step of direction = (row_step, col_step),
        count how many consecutive cells are owned by the same player as the starting cell.
        """
        ...
        while (0 <= row < self.board_height and 0 <= col < self.board_width
                and (self.get_cell(row, col) == playerid
                or (potential and self.get_cell(row, col) == 0))):
    ...
    def _chain_sets_from_cell(self, row, col, potential=False):
        """ Return the max-length chain containing this cell """
        return [ tuple(x) for x in [
                reverse(self._contig_vector_cells(row, col, (1,1), potential)) + [(row, col)] + self._contig_vector_cells(row, col, (-1,-1), potential),
                 reverse(self._contig_vector_cells(row, col, (1,0), potential)) + [(row, col)] + self._contig_vector_cells(row, col, (-1,0), potential),
                reverse(self._contig_vector_cells(row, col, (0,1), potential)) + [(row, col)] + self._contig_vector_cells(row, col, (0,-1), potential),
                reverse(self._contig_vector_cells(row, col, (-1,1), potential)) + [(row, col)] + self._contig_vector_cells(row, col, (1,-1), potential) 
                 ] ]

    def chain_cells(self, playerid, potential=False):
        """
        ...
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
                if self.get_cell(i,j) == playerid:
                    retVal.update( self._chain_sets_from_cell(i,j, potential) )
    ...
    def is_win(self):
        ...
        if self.longest_streak:
            if self.num_tokens_on_board() < 20:
                return 0
            current_player_streak =  self.longest_chain(self.get_current_player_id())
            other_player_streak = self.longest_chain(self.get_other_player_id())
            if current_player_streak > other_player_streak:
                return self.get_current_player_id()
            elif other_player_streak > current_player_streak:
                return self.get_other_player_id()
            return 0
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._board_array, self.connect_k, self.longest_streak,board_already_won=self._is_win, current_player = self.get_current_player_id())
...
class ConnectFourRunner(object):
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def run_game(self, verbose=True):
        ...
        if win_for_player:
            print "Win for %s!" % self._board.board_symbol_mapping[win_for_player]
            self._do_gameend()
